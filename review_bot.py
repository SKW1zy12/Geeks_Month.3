...
start_buttons = [
    ...
    types.KeyboardButton("Отправь сообщение")
]
...
@dp.message_handler(text='Отправь сообщение')
async def send_anonim_message(message:types.Message):
    await message.answer(f"Вы хотите отправить анонимнное сообщение", reply_markup=anonim_select_keyboard)
...
async def  select_yes_answer(message:types.CallbackQuery):
    await bot.edit_message_text(chat_id=message.message.chat.id, message_id=message.message.message_id,
                                text = "Напишите свое сообщение и мы его отправим в группу")
